...
// Optional: Use this to switch between localhost and deployed backend
const BASE_URL = location.hostname === 'localhost'
    ? 'http://localhost:8080'
    : 'https://hangman-lt7u.onrender.com';
...
function handleGuess(letter) {
    fetch(`${BASE_URL}/guess`, {
        ...
        .then(data => {
            ...
            document.getElementById('message').className = data.message === 'Correct guess!' 
                ? 'correct' 
                : data.message === 'Wrong guess!' 
                ? 'wrong' 
                : '';
